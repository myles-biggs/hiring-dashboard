...
// POST — queues background AI vetting for all unvetted candidates via Inngest
export async function POST(
  ...
  const vettedIds = new Set(alreadyVetted.map((c) => c.workableCandidateId));
  const toVetCount = activeCandidates.filter((c) => !vettedIds.has(c.id)).length;

  if (toVetCount === 0) {
    return NextResponse.json({ queued: false, toVet: 0, message: "All candidates already scored" });
  }
  ...
  await inngest.send({
    name: "hiring/vet-all.requested",
    data: { shortcode, briefId: brief?.id ?? null },
  });

  return NextResponse.json({ queued: true, toVet: toVetCount });
}
